...
// are scaled by 'bscale'.  In-place modifications are allowed.
void svecd_add_i0(svecd_t *x, svecd_t *a, svecd_t *b, double bscale, int a0, int b0)
{
    int aidx = 0, bidx = 0;
    int ai, bi;
...
smatd_t *smatd_multiply_dpmethod(smatd_t *a, smatd_t *b)
{
    ...
    for (int col = 0; col < xcols; col++) {
        svecd_t *bcol = smatd_get_column(b, col);
...
// contributions from C and then recursively factor C.
    ...
    for (int i = 0; i < a->nrows; i++) {
        ...
        for (int pos = 0; pos < urowi->nz; pos++) {
            ...
            if (j > i) {
                ...
                svecd_add_i0(urowj, urowj, urowi, -s, i+1, i+1);
            }
        }
    }
