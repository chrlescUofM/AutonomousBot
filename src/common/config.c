...
typedef struct _parser_file_t parser_file_t;
typedef struct _config_element_t config_element_t;

typedef int (*GetChFunc)(parser_t *);
 ...
 */
static int
get_token (parser_t *p, ConfigToken *tok, char *str, int len)
{
...
static config_element_t *
new_element (const char *name)
{
    config_element_t *el;

    el = malloc (sizeof (config_element_t));
    memset (el, 0, sizeof (config_element_t));
    if (name)
        el->name = strdup (name);
...
static void
free_element (config_element_t *el)
{
    free (el->name);
    config_element_t *child, *next;
...
static int
add_child (parser_t *p, config_element_t *el, config_element_t *child)
{
    config_element_t **nptr;
...
static int
add_value (parser_t *p, config_element_t *el, const char *str)
{
    int n = el->num_values;
 ...
 */
static int
parse_array (parser_t *p, config_element_t *el)
{
    ConfigToken tok;
    char str[256];
 ...
 */
static int
parse_right_side (parser_t *p, config_element_t *el)
{
    ConfigToken tok;
    char str[256];
...
static int
parse_container (parser_t *p, config_element_t *cont, ConfigToken end_token)
{
    ConfigToken tok;
    char str[256];
    config_element_t *child = NULL;
...
static int
print_array (config_element_t *el, int indent)
{
...
static int
print_container (config_element_t *el, int indent)
{
    config_element_t *child;
 ...
 * output. */
int
config_print (config_t *conf)
{
    config_element_t *child, *root;
...
config_parse_default (void)
{
    ...
    config_t *conf = config_parse_file (f, path);
    fclose (f);
    return conf;
}
...
static config_element_t *
find_key (config_element_t *el, const char *key, int inherit)
{
...
int
config_has_key (config_t *conf, const char *key)
{
    return (find_key (conf->root, key, 1) != NULL);
}

int
config_get_num_subkeys (config_t *conf, const char *containerKey)
{
  config_element_t *el = conf->root;
...
char **
config_get_subkeys (config_t *conf, const char *containerKey)
{
    config_element_t *el = conf->root;
    if ((NULL != containerKey) && (0 < strlen(containerKey)))
        el = find_key (conf->root, containerKey, 1);
...
int
config_get_int (config_t *conf, const char *key, int *val)
{
    config_element_t *el = find_key (conf->root, key, 1);
...
int
config_get_boolean (config_t *conf, const char *key, int *val)
{
    config_element_t *el = find_key (conf->root, key, 1);
    if (!el || el->type != ConfigArray || el->num_values < 1) {
        return -1;
...
int
config_get_double (config_t *conf, const char *key, double *val)
{
    config_element_t *el = find_key (conf->root, key, 1);
    if (!el || el->type != ConfigArray || el->num_values < 1) {
        return -1;
...
double config_get_double_or_fail (config_t *conf, const char *key)
{
    double v;
...
int
config_get_str (config_t *conf, const char *key, char **val)
{
    config_element_t *el = find_key (conf->root, key, 1);
    if (!el || el->type != ConfigArray || el->num_values < 1) {
        return -1;
...
char *
config_get_str_or_fail (config_t *conf, const char *key)
{
    char *str;
...
int
config_get_int_or_default (config_t *conf, const char *key, int def)
{
    int val;
...
int
config_get_boolean_or_default (config_t *conf, const char *key, int def)
{
    int val;
...
double
config_get_double_or_default (config_t *conf, const char *key, double def)
{
    double val;
...
char *
config_get_str_or_default (config_t *conf, const char *key, char *def)
{
    char *str;
    if (config_get_str (conf, key, &str) == 0)
        return str;
...
int
config_get_int_array (config_t *conf, const char *key, int *vals, int len)
{
    config_element_t *el = find_key (conf->root, key, 1);
...
int
config_get_boolean_array (config_t *conf, const char *key, int *vals, int len)
{
    config_element_t *el = find_key (conf->root, key, 1);
    if (!el || el->type != ConfigArray) {
        return -1;
...
int
config_get_double_array (config_t *conf, const char *key, double *vals, int len)
{
    config_element_t *el = find_key (conf->root, key, 1);
    if (!el || el->type != ConfigArray) {
        return -1;
...
int
config_get_str_array (config_t *conf, const char *key, char **vals, int len)
{
    config_element_t *el = find_key (conf->root, key, 1);
    if (!el || el->type != ConfigArray) {
        return -1;
...
int
config_get_array_len (config_t *conf, const char *key)
{
    config_element_t *el = find_key (conf->root, key, 1);
    if (!el || el->type != ConfigArray) {
        return -1;
...
char **
config_get_str_array_alloc (config_t *conf, const char *key)
{
    config_element_t *el = find_key (conf->root, key, 1);
    if (!el || el->type != ConfigArray) {
        return NULL;
...
void
config_str_array_free (char **data)
{
    int idx = 0;
    while (data[idx] != NULL)
        free(data[idx++]);
...
static config_element_t *
create_key (config_element_t *el, const char *key)
{
  size_t len = strcspn (key, ".");
  char str[len+1];
  memcpy (str, key, len);
...
static int
set_value (config_t *conf, const char *key, const char *val)
{
  config_element_t *el = find_key (conf->root, key, 0);
  if (el == NULL)
...
int
config_set_boolean (config_t *conf, const char *key, int val)
{
...
int
config_set_str (config_t *conf, const char *key, char *val)
{
...
int
config_set_int_array (config_t *conf, const char *key, int *vals, int len)
{
  char *str;
  char single_val[16];
  int string_len = 1;
...
int
config_set_boolean_array (config_t *conf, const char *key, int *vals, int len)
{
  char *str;
  char single_val[16];
  int string_len = 1;
...
int
config_set_double_array (config_t *conf, const char *key, double *vals, int len)
{
  char *str;
  char single_val[32];
  int string_len = 1;
...
int
config_set_str_array (config_t *conf, const char *key, char **vals, int len)
{
  char *str;
  char single_val[256];
  int string_len = 1;
